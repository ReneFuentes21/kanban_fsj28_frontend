...
import { STATUS_BG_COLORS, STATUS_BORDER_COLORS, EmptyStateIcon, PlusIcon, ItemTypes } from '../constants';

const Column = ({ column, columns, onAddTask, onDeleteTask, onEditTask, onSaveTask, onMoveTask }) => { 
    ...
    const [{ isOver }, drop] = useDrop(() => ({
        ...
        drop: (item) => {
            if (item.currentColumn !== column.id) {
                onMoveTask(item.id, item.currentColumn, column.id);
            }
        },
        collect: (monitor) => ({
    ...
    return (
        <div 
            ref={drop}
            className={`flex flex-col rounded-xl shadow-sm p-4 border ${borderColor} ${bgColor} ${
                isOver ? 'ring-2 ring-blue-400 bg-blue-50 dark:bg-blue-900/20 transition-all duration-200' : ''
            ...
            <div className="flex-grow space-y-4 overflow-y-auto pr-1">
                {column.tasks.length > 0 ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-32 text-center">
                        ...
                        <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Suelta tareas aquí</p>
                    </div>
                )}
            </div>
